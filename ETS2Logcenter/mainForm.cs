...
namespace ETS2Logcenter
{
    public partial class mainForm : Form
    {
        ...
        private void openFile(string path)
        {
            ...
            try
            {
                if (File.Exists(path))
                {
                    ...
                    dateOfLog = String.Format("{0}-{1}-{2}", rgx.Matches(path)[0].Groups["day"].Value, rgx.Matches(path)[0].Groups["month"].Value, rgx.Matches(path)[0].Groups["year"].Value);
                    this.Text = String.Format("ETS2 Logcenter - [{0}]", dateOfLog);
        ...
        private void btnContribute_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("No data to contribute.",
                    "Error",
        ...
        private void uploadData()
        {
            ...
            pbContribute.Maximum = lstDrivers.Items.Count + 1;
            string constr = "Data Source=localhost;Initial Catalog=ets2logcenter;User ID=root;Password=";
            using (MySqlConnection con = new MySqlConnection(constr))
            {
                ...
                try
                {
                    ...
                    {
                        using (MySqlCommand cmdPlayerlog = new MySqlCommand("INSERT IGNORE INTO playerlog (date, game_id, name, tag, ets2mp_id) VALUES(@Date, @Game_id, @Name, @Tag, @Ets2mp_id)", con))
                        {
                            cmdPlayerlog.Parameters.Add(new MySqlParameter("Date", dateOfLog.ToString()));
                            cmdPlayerlog.Parameters.Add(new MySqlParameter("Game_id", lstDrivers.Items[i].SubItems[4].Text));
                            cmdPlayerlog.Parameters.Add(new MySqlParameter("Name", lstDrivers.Items[i].SubItems[3].Text));
                            cmdPlayerlog.Parameters.Add(new MySqlParameter("Tag", lstDrivers.Items[i].SubItems[2].Text));
                            cmdPlayerlog.Parameters.Add(new MySqlParameter("Ets2mp_id", lstDrivers.Items[i].SubItems[5].Text));

                            cmdPlayerlog.ExecuteNonQuery();
                        }
                        pbContribute.Value += 1;
                    }
                    using (MySqlCommand cmdSubmitlog = new MySqlCommand("INSERT INTO submitlog(submittedrecords) VALUES(@submittedrecords)", con))
                    {
                        cmdSubmitlog.Parameters.Add(new MySqlParameter("submittedrecords", Convert.ToString(lstDrivers.Items.Count + 1)));
                        cmdSubmitlog.ExecuteNonQuery();
                    }

                    MessageBox.Show("Your data is added.", "Thank you", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
                }
                catch (Exception e)
                {
